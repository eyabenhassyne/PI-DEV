...
{
    // 1. El Index m-nadfma fiha el Recherche
    ...
    public function index(Request $request, EvenementRepository $evenementRepository): Response
    {
        ...
        if ($searchTerm) {
            // Thabbet hna: ken el champ esmou "title" mouch "Title" f'el Entity
...
#[Route(name: 'app_evenement_index', methods: ['GET'])]
    ...
    {
        // 1. Nakhou el parameters mel URL
        $searchTerm = $request->query->get('q');
        $sortBy = $request->query->get('sort', 'id'); // ID hiya el default
        $direction = $request->query->get('direction', 'ASC'); // ASC hiya el default

        // 2. Nabni el Query
        $queryBuilder = $evenementRepository->createQueryBuilder('e');

        // 3. Ken famma recherche, n-zidouha
        if ($searchTerm) {
            $queryBuilder->andWhere('e.title LIKE :term')
                         ->setParameter('term', '%'.$searchTerm.'%');
        }

        // 4. N-zidou el Tri (Thabbet mlih f'el syntaxe hna)
